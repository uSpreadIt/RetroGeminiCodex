...
### Fixed
- Backup service no longer crashes the server when `/data` is not writable (fallback to `/tmp/backups`)
- TOCTOU race condition in backup lock file acquisition
- Insecure temp directory creation in backup tests
